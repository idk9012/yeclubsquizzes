...
router.get('/review-latest/:quizId', async (req, res) => {
  try {
    ...
    // Get the latest completed submission for this quiz
    ...
    const reviewData = {
      ...
      questions: questions.map((question, index) => {
        ...
        if (userAnswer && userAnswer.length > 0 && userAnswer[0]) {
    ...
    res.status(500).json({ error: 'Failed to fetch quiz review data' });
  }
});
